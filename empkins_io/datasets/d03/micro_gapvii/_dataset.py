...
class MicroBaseDataset(Dataset):
    ...
    def cortisol(self) -> pd.DataFrame:
        cortisol_path = self.base_path.joinpath(
            "data_tabular/saliva/cortisol/cleaned/cortisol_cleaned.csv"
        )
    ...
    def sex(self) -> pd.DataFrame:
        return load_long_format_csv(
            self.base_path.joinpath("data_tabular/extras/processed/sex.csv"),
            index_cols=["subject"],
        )
    ...
    def condition_order(self) -> pd.DataFrame:
        return load_long_format_csv(
            self.base_path.joinpath(
                "data_tabular/extras/processed/condition_order.csv"
            ),
            index_cols=["subject"],
        )
    ...
    def sit_stand(self) -> pd.DataFrame:
        return load_long_format_csv(
            self.base_path.joinpath("data_tabular/extras/processed/sit_stand.csv"),
            index_cols=["subject"],
        )
    ...
    def condition_day_mapping(self) -> pd.DataFrame:
        return load_long_format_csv(
            self.base_path.joinpath(
                "data_tabular/extras/processed/day_condition_mapping.csv"
            ),
    ...
    def biopac(self) -> pd.DataFrame:
        ...
        if self.is_single(["subject", "condition"]):
            if self.phase_fine and (
                len(self.index["phase"]) not in [1, len(self.PHASE_FINE)]
            ):
                warnings.warn(
                    "Biopac data can only be accessed for all phases or one specific phase!"
                )
            elif not self.phase_fine and (
                len(self.index["phase"]) not in [1, len(self.PHASE_COARSE)]
            ):
                warnings.warn(
                    "Biopac data can only be accessed for all phases or one specific phase!"
                )
    ...
    def timelog(self) -> pd.DataFrame:
        ...
        raise ValueError(
            "Timelog can only be accessed for a single participant and a single condition!"
        )
    ...
    def emrad(self) -> pd.DataFrame:
        ...
        if self.is_single(["subject", "condition"]):
            if self.phase_fine and (
                ...
                warnings.warn(
                    "Emrad data can only be accessed for all phases or one specific phase!"
                )
            elif not self.phase_fine and (
                len(self.index["phase"]) not in [1, len(self.PHASE_COARSE)]
            ):
    ...
    def emrad_biopac_synced_and_sr_aligned(self):
        ...
        self.assert_is_single(
            ["subject", "condition"], "emrad_biopac_synced_and_sr_aligned"
        )
        ...
        current_vp = list(
            set(self.create_group_labels(label_cols=["subject", "condition"]))
        )[0][0]
        ...
        for radar_sensor in raw_radar_data.columns.get_level_values(0).unique():
            synced_data.add_dataset(
                radar_sensor, raw_radar_data.xs(radar_sensor, axis=1), "Sync_In", fs
            )

        # Add the biopac data
        raw_biopac_data = self.biopac
        synced_data.add_dataset(
            "Biopac", raw_biopac_data, "sync", self._sampling_rates["biopac"]
        )
        ...
        for (
            ...
            aligned_dataset_vals,
        ) in synced_data.datasets_aligned.items():
            if "rad" in aligned_dataset_name:
                resampled_data.add_dataset(
                    aligned_dataset_name, aligned_dataset_vals, "Sync_In", fs
                )
        ...
        for name in resampled_data.datasets:
            setattr(
                resampled_data,
                f"{name}_resampled_",
                resampled_data.datasets_resampled[f"{name}_resampled_"][
                    start_time:end_time
                ],
            )
