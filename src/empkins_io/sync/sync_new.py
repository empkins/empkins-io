...
class BaseSync:
    """Base class holding functionality shared by all sync types.

    Contains dataset registration/validation, resampling, plotting, and the
    accessors for resampled/cut/aligned datasets. Subclasses add the alignment
    logic that is specific to a synchronization strategy.
    """
...
class SyncedDataset(BaseSync):
    """Full synchronization implementation.

    Inherits the shared functionality from :class:`BaseSync` and adds all
    additional implemented functionality: the peak-detection alignment path
    (trigger / edge / clock sync types) as well as the m-sequence
    cross-correlation path.
    """
...
class SyncedDatasetMSequence(BaseSync):
    """Synchronization for the ``m-sequence`` sync type only.

    Inherits the shared functionality from :class:`BaseSync` and adds only the
    cross-correlation-based alignment specific to m-sequence synchronization.

    Note: this intentionally duplicates the m-sequence logic that currently also
    lives in :class:`SyncedDataset`.
    """
