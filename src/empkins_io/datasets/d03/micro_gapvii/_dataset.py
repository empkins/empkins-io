...
class MicroBaseDataset(Dataset):
    ...
    def face_video_path_sliced(self) -> Path:
        if self.is_single(["subject", "condition", "phase"]):
            ...
            condition = self.condition
            path = _build_data_path(self.base_path, participant_id, condition)
            path = path.joinpath(f"video/face/cleaned/video_face_{participant_id}_{condition}_sliced.mp4")
            return path
        else:
            raise ValueError("Only valid for a single participant and condition (and phase)!")

    @property
    def timelog_video(self):
        if self.is_single(["subject", "condition"]):
            timelog_all = pd.read_excel(self.data_tabular_path.joinpath("_extras/raw/video_labels.xlsx"))
            timelog_all = timelog_all.set_index(["subject", "condition"])
            timelog_all = timelog_all.drop(columns="Notes")
            timelog_all = timelog_all.astype(str)

            return timelog_all.loc[self.subject, self.condition].apply(_time_convert_to_sec)
        else:
            raise ValueError("Timelog can only be accessed for a single participant and a single condition at once!")
    ...
    def _get_nilspod_data(self, subject_id: str, condition: str, phase: str) -> pd.DataFrame:
