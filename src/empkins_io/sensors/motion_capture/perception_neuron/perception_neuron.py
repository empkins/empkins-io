...
class PerceptionNeuronDataset:
    ...
    @property
    def bvh_data(self) -> BvhData | None:
        """Return BVH data."""
        return self._data_dict.get("bvh", None)

    @property
    def calc_data(self) -> CalcData | None:
        """Return Calc data."""
        return self._data_dict.get("calc", None)

    @property
    def center_mass_data(self) -> CenterOfMassData | None:
    ...
    def data_as_df(self, data_formats: str_t | None = None, index: str | Sequence[str] | None = None) -> pd.DataFrame:
