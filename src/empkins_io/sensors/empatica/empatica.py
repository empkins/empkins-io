...
class EmpaticaDataset:
    ...
    def gyro(self) -> pd.DataFrame:
        raise ValueError(
            "Probably there is no gyro data in the Empatica dataset."
        )  # TODO check this
    ...
    def data_as_df(self, sensor: str) -> pd.DataFrame:
        ...
        if sensor not in self._sensor_dict:
            raise ValueError(
                f"Supplied sensor ({sensor}) is not allowed. Allowed values: {self._sensor_dict.keys()}"
            )
    ...
    def _data_as_df_folder(self, sensor: str) -> pd.DataFrame:
        ...
        start_time_unix = self._raw_data[list(self._raw_data.keys())[0]]["rawData"][
            sensor
        ]["timestampStart"]
        ...
        for file in self._raw_data:
            ...
            df = pd.DataFrame(
                {
                    f"{sensor}_{channel}": sensor_dict[channel]
                    for channel in self._sensor_dict[sensor]
                }
            )

            df = self._add_index_for_stitching(
                df, sensor_dict["samplingFrequency"], sensor_dict["timestampStart"]
            )
            ...
            if prev_last_timestamp is not None:
                ...
                if (df.index[0] - prev_last_timestamp) > (
                    pd.Timedelta(seconds=2 / sensor_dict["samplingFrequency"])
                ):
                    raise ValueError(
                        f"Gap between files detected. Please check the files in {self.path}."
                    )
